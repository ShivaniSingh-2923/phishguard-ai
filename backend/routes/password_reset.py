...
def _verify_otp_code(email: str, plain_otp: str) -> None:
    ...
    Raises ValueError with a code prefix for the caller to parse:
      __not_found__   no OTP record exists
      __expired__     OTP past its expiry time
      __max_attempts__ too many wrong tries
      __invalid__     wrong code (increments attempt counter)
    """
    ...
    if doc.get("used"):
        raise ValueError("__not_found__")  # treat used OTP same as not found
    ...
    # Mark as verified so /reset-password can confirm step 2 was done
    otp_collection.update_one(
        {"email": email},
        {"$set": {"verified": True}},
    )
